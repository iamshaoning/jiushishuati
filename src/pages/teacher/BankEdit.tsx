    ...
    return (
      <>
        <div className="card p-12 text-center">
          <div className="text-gray-500">题库不存在或无访问权限</div>
    ...
    <>
            ...
            {editMeta ? (
              ...
              <>
                ...
                <div className="flex items-center gap-3 mt-2 text-xs text-gray-400">
                  ...
                  <span>·</span>
                  <span>创建于 {new Date(bank.created_at).toLocaleDateString('zh-CN')}</span>
                </div>
              </>
            )}
          </div>
      ...
      <div className="space-y-3">
        {questions.length === 0 && !addingNew && (
          <div className="card p-10 text-center text-sm text-gray-400">暂无题目，点击右上角添加</div>
        )}
        {questions.map((q, i) => (
            ...
            {editingId === q.id ? (
              <QuestionEditor
                ...
                onChange={(nq) => {
                  // 实时更新本地缓存
                  setQuestions((arr) =>
                    arr.map((x) => (x.id === q.id ? { ...x, ...nq } as Question : x)),
                  );
                }}
